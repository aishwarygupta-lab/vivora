...
Usage:
    python scripts/seed_demo.py --token <jwt>                 # 3 demo avatars
    python scripts/seed_demo.py --token <jwt> --with-voices   # + a cloned demo voice each
    python scripts/seed_demo.py --token <jwt> --api http://localhost:8000

Get a token by logging in, e.g.:
    curl -s -X POST http://localhost:8000/api/v1/users/login \
      -d "username=demo@vivora.app&password=VivoraDemo123!" | python -c \
...
VOICE_SAMPLE_TEXT = (
    "Hi there! I'm one of the demo voices for the Vivora platform. "
...
def fetch_face(timeout: int = 15) -> bytes:
    ...
    resp = requests.get(FACE_SOURCE, timeout=timeout, headers={"User-Agent": "vivora-seed"})
...
def placeholder_face(label: str) -> bytes:
    ...
    d.text((20, 470), f"Vivora demo — {label}", fill=(200, 200, 220))
    buf = io.BytesIO()
...
def main() -> int:
    ...
    ap.add_argument("--token", required=True, help="Bearer token — all endpoints require auth")
    args = ap.parse_args()
