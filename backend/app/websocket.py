...
class ConnectionManager:
    ...
    async def _resolve_local_image(self, avatar) -> str:
        """Return a local FS path to the avatar's DRIVING media (image or video),
        downloading from S3 if needed.

        s3_key points at the source that MuseTalk animates: image.jpg for a photo
        avatar, source.mp4 for a video avatar. We preserve that suffix so a video
        avatar isn't cached (and fed to the animator) as a bogus .jpg.
        """
        suffix = Path(avatar.s3_key).suffix or ".jpg"
