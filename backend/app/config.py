...
class Settings(BaseSettings):
    ...
    # Point at a MinIO / S3-compatible server (e.g. http://minio:9000) for
    # local/Docker parity. Empty → real AWS S3. Only used when USE_LOCAL_STORAGE=false.
    ...
    # Avatar Engine
    # auto = MuseTalk lip-sync on a CUDA GPU, lightweight simple path on CPU (recommended).
    # musetalk = always MuseTalk (very slow / impractical on CPU).  simple = never lip-sync.
    AVATAR_ENGINE: str = "auto"  # auto, musetalk, simple
    AVATAR_RESOLUTION: int = 512
    AVATAR_FPS: int = 25
    MUSETALK_PATH: str = "models/MuseTalk"

    # Avatar source media (photo OR video). A video avatar drives real head
    # motion + lip-sync; a photo still lip-syncs the mouth on a static frame.
    AVATAR_ALLOW_VIDEO: bool = True
    AVATAR_MAX_IMAGE_MB: int = 10
    AVATAR_MAX_VIDEO_MB: int = 100
    # We only turn the first N seconds of a driving video into the frame bank
    # (also the in-browser recording cap). Longer clips add cost, not realism.
    AVATAR_MAX_VIDEO_SECONDS: int = 20
    AVATAR_VIDEO_EXTENSIONS: Union[List[str], str] = ["mp4", "webm", "mov", "m4v"]
    ...
    @field_validator("CORS_ORIGINS", "ALLOWED_EXTENSIONS", "AVATAR_VIDEO_EXTENSIONS", mode="before")
    @classmethod
