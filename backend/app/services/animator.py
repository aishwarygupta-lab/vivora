...
class AvatarAnimator:
    ...
    Supported engines (set AVATAR_ENGINE in .env):
      - auto     : MuseTalk on a CUDA GPU, otherwise 'simple' (recommended default)
      - musetalk : MuseTalk V1.5 — persistent worker (models loaded once); GPU-only in practice
      - simple   : ffmpeg — static image (photo) or looped clip (video) + audio, no lip-sync
    """
    ...
    async def initialize(self):
        ...
        # "auto" (recommended default): real lip-sync (MuseTalk) on a CUDA GPU,
        # but on CPU use the lightweight simple path instead of loading a diffusion
        # model that would take minutes per reply (and time out) — e.g. on a Mac.
        if self.engine == "auto":
            if self.device == "cuda":
                logger.info("AVATAR_ENGINE=auto → CUDA GPU detected, using MuseTalk lip-sync.")
                self.engine = "musetalk"
            else:
                logger.warning(
                    "AVATAR_ENGINE=auto → no CUDA GPU; using 'simple' (static/clip + audio, "
                    "no lip-sync). Lip-sync activates automatically on a GPU. "
                    "Set AVATAR_ENGINE=musetalk to force it on CPU (very slow)."
                )
                self.engine = "simple"
    ...
    async def _ensure_worker(self) -> asyncio.subprocess.Process:
        ...
        # The worker ships with our backend (not inside the gitignored MuseTalk
        # clone, or it would vanish on a fresh checkout). We run it with the
        # MuseTalk dir as cwd + on PYTHONPATH so its `import musetalk…` resolves.
        worker_script = Path(__file__).resolve().parent / "musetalk_worker.py"
    ...
    async def _animate_simple(
        ...
        """Combine the avatar with the reply audio via FFmpeg. No lip-sync.

        For a VIDEO avatar we loop the recorded clip for the length of the reply
        so the face still moves and speaks (much more alive than a frozen frame);
        for a PHOTO avatar we render the still image + audio.
        """
        video_exts = {".mp4", ".webm", ".mov", ".m4v", ".avi", ".mkv"}
        is_video = Path(avatar_path).suffix.lower() in video_exts
